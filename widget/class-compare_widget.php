...
class WPEC_Compare_Widget extends WP_Widget
{
	...
	function __construct() {
		$widget_ops = array(
			'classname'   => 'wpec_compare_widget',
		);
		parent::__construct('wpec_compare_widget', __('WPEC Compare Products', 'wp-ecommerce-compare-products' ), $widget_ops);
	}
	...
	function widget($args, $instance) {
		...
		echo $before_widget;
		if ($wpec_compare_widget_title_style['enable_widget_title_customized'] == 1) {
			$before_title = '<div id="compare_widget_title_container"><span id="compare_widget_title_text">';
			$after_title = '</span></div><div style="clear:both"></div>';
		...
		if ( $title != '')
			...
			echo $before_title . __( 'Compare Products', 'wp-ecommerce-compare-products' ).' <span class="total_compare_product_container">'.$before_total_text.'<span id="total_compare_product">'.$total_compare_product.'</span>'.$after_total_text.'</span>' . $after_title;
	...
	function form($instance) {
        ...
        <p>
          	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title', 'wp-ecommerce-compare-products' ); ?> :

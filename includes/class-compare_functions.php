...
class WPEC_Compare_Functions
{
	...
	public static function get_compare_list_html_widget() {
		...
		if (is_array($compare_list) && count($compare_list)>0) {
			...
			foreach ($compare_list as $product_id) {
				$thumbnail_html = '';
				if ($wpec_compare_widget_thumbnail_style['activate_thumbnail'] == 1) {
			...
			if ($wpec_compare_widget_clear_all_style['clear_all_button_type'] == 'button') {
				$widget_clear_all_class = 'wpec_compare_clear_all_button';
				$widget_clear_all_custom_class = '';
				$widget_clear_all_text = $wpec_compare_widget_clear_all_style['clear_all_button_text'];
			}
			...
			if ($wpec_compare_widget_button_style['compare_widget_button_type'] == 'link') {
				$widget_button_class = 'wpec_compare_widget_link_go';
				$widget_button_custom_class = '';
				$widget_button_text = $wpec_compare_widget_button_style['compare_widget_link_text'];
			}
	...
	public static function get_compare_list_html_popup() {
		...
		if ($wpec_compare_addtocart_style['addtocart_button_type'] == 'button') {
			$add_to_cart_button_class = 'add_to_cart_button_type';
			$custom_class = '';
			$add_to_cart_text = $wpec_compare_addtocart_style['addtocart_button_text'];
		}
		...
		if (is_array($compare_list) && count($compare_list)>0) {
			...
			foreach ($compare_list as $product_id) {
				...
				$html .= '<td class="first_row column_'.$i.'"><div class="td-spacer"><div class="wpec_compare_popup_remove_product_container"><a class="wpec_compare_popup_remove_product" rel="'.$product_id.'" style="cursor:pointer;">'.__('Remove', 'wp-ecommerce-compare-products' ).' <img src="'.$wpec_compare_basket_icon.'" border=0 /></a></div>';
				$html .= '<div class="compare_image_container">'.$image_src.'</div>';
				$html .= '<div class="compare_product_name">'.$product_name.'</div>';
				$html .= '<div class="compare_avg_rating">'.__( 'Avg. Customer Rating', 'wp-ecommerce-compare-products' ).':<br />'.wpsc_product_existing_rating( $product_id ).'</div>';
				if ($wpec_compare_product_prices_style['disable_product_price'] != 1 && $wpec_compare_product_prices_style['product_price_position'] != 'bottom') $html .= '<div class="compare_price">'.$products_prices[$product_id].'</div>';
				if ($wpec_compare_addtocart_style['disable_product_addcart'] != 1 ) {
					if ((get_option('hide_addtocart_button') == 0)) {
						...
						if (wpsc_product_external_link($product_id) != '') {
							...
							$html .= sprintf('<a href="%s" rel="nofollow" class="button add_to_cart_button %s %s" target="_blank">%s</a>', $cart_url, $add_to_cart_button_class, $custom_class, wpsc_product_external_link_text( $product_id, __( 'Buy Now', 'wp-ecommerce-compare-products' ) ) );
			...
			if (is_array($compare_fields) && count($compare_fields)>0) {
				...
				$have_audio_type = false;
				$add_audio_player_scripts = false;
				foreach ($compare_fields as $field_data) {
					$j++;
					
					$hide_in_print_class = '';
					if ( in_array( $field_data->field_type, array( 'wp-video', 'wp-audio' ) ) ) {
						$hide_in_print_class = 'hide_in_print';
						$have_audio_type = true;
					}
					if ( $have_audio_type === true && $add_audio_player_scripts === false ) {
						$add_audio_player_scripts = true;
						// Add Media lib script for support audio player in comparison table
						add_action( 'wpeccp_comparison_page_footer', array( 'WPEC_Compare_Hook_Filter', 'wpeccp_add_wp_media_lib_scripts' ) );
                    	$extra_scripts .= '<script type="text/javascript" src="'.$site_url.'/wp-includes/js/mediaelement/mediaelement-and-player.min.js"></script>';
						$extra_scripts .= '<script type="text/javascript" src="'.$site_url.'/wp-includes/js/mediaelement/wp-mediaelement.js"></script>';
					}
					$html .= '<tr class="row_'.$j.' '. $hide_in_print_class .'">';
					...
					foreach ($compare_list as $product_id) {
						...
						if ( $field_data->field_type == 'wp-video' ) {
							if ( $field_value != $wpec_compare_table_content_style['empty_text'] ) {
								$field_value = do_shortcode( '[embed width=220 height=165]'. trim( strip_shortcodes( $field_value ) ) . '[/embed]' );
							}
						} elseif ( $field_data->field_type == 'wp-audio' ) {
							if ( $field_value != $wpec_compare_table_content_style['empty_text'] ) {
								$field_value = do_shortcode( '[audio src="'. trim( strip_shortcodes( $field_value ) ) . '" ]' );
							}
						}
				...
				if ($wpec_compare_product_prices_style['disable_product_price'] != 1 && $wpec_compare_product_prices_style['product_price_position'] != 'top') {
					$j++;
					if ($j>2) $j=1;
	...
	public static function printPage($link, $total = 0,$currentPage = 1,$div = 3,$rows = 5, $li = false, $a_class= ''){
		...
		if($currentDiv) {	
			if($li){
				...
				$sPage .= '<li><span class="pagenav"><a title="" class="page-numbers '.$a_class.'" href="'.add_query_arg('pp', $currentDiv*$div, $link).'">'.__("Back", 'wp-ecommerce-compare-products' ).'</a></span></li>';	
			}else{
				$sPage .= '<a title="" class="page-numbers '.$a_class.'" href="'.add_query_arg('pp', 1, $link).'">&laquo;</a> ';	
				$sPage .= '<a title="" class="page-numbers '.$a_class.'" href="'.add_query_arg('pp', $currentDiv*$div, $link).'">'.__("Back", 'wp-ecommerce-compare-products' ).'</a> ';	
			}
		}
		...
		if($currentDiv < $nDiv - 1){	
			if($li){	
				$sPage .= '<li><span class="pagenav"><a title="" class="page-numbers '.$a_class.'" href="'.add_query_arg('pp', ((($currentDiv+1)*$div)+1), $link).'">'.__("Next", 'wp-ecommerce-compare-products' ).'</a></span></li>';	
				$sPage .= '<li><span class="pagenav"><a title="" class="page-numbers '.$a_class.'" href="'.add_query_arg('pp', (($nDiv*$div )-2), $link).'">&raquo;</a></span></li>';	
			}else{
				$sPage .= '<a title="" class="page-numbers '.$a_class.'" href="'.add_query_arg('pp', ((($currentDiv+1)*$div)+1), $link).'">'.__("Next", 'wp-ecommerce-compare-products' ).'</a> ';	
	...
	public static function upgrade_version_2_1_0() {
		WPEC_Compare_Functions::create_page( esc_sql( 'product-comparison' ), '', __('Product Comparison', 'wp-ecommerce-compare-products' ), '[product_comparison_page]' );
	...
	public static function pro_upgrade_version_2_1_5() {
		$wpec_compare_product_page_settings = get_option('wpec_compare_product_page_settings', array() );

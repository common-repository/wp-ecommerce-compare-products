...
class WPEC_Compare_MetaBox
{
	public static function compare_meta_boxes() {
		...
		add_meta_box( 'wpec_compare_feature_box', __('Compare Feature Fields', 'wp-ecommerce-compare-products' ), array('WPEC_Compare_MetaBox', 'wpec_compare_feature_box'), $pagename, 'normal', 'high' );
	}
	...
	public static function wpec_compare_feature_box() {
		...
		<label for='deactivate_compare_feature' class='small'><?php _e( "Activate Compare Feature for this Product", 'wp-ecommerce-compare-products' ); ?></label>
		<div class="compare_feature_activate_form" style=" <?php if ( $deactivate_compare_feature != 'no') { echo 'display:none;';} ?>">
            <p><label for='compare_category' class='small'><?php _e( "Select Compare Category for this Product", 'wp-ecommerce-compare-products' ); ?></label> : 
...
?>
                ...
                <tr class="form-field">
               	...
               	<?php
					...
					switch($field_data->field_type){
						...
						case "wp-video":
							echo '<textarea style="width:400px" name="_wpsc_compare_'.$field_data->field_key.'" id="'.$field_data->field_key.'">'.$field_value.'</textarea>';
							echo '<div class="description">'.__( 'Input Video URL - Supported formats Youtube, Vimeo, WordPressTV', 'wp-ecommerce-compare-products' ).'</div>';
							break;
							
						case "wp-audio":
							echo '<textarea style="width:400px" name="_wpsc_compare_'.$field_data->field_key.'" id="'.$field_data->field_key.'">'.$field_value.'</textarea>';
							echo '<div class="description">'.__( 'Input Audio URL - Supported formats .mp3, .m4a, .ogg, .wav file', 'wp-ecommerce-compare-products' ).'</div>';
							break;
						...
						case "drop-down":
							...
							echo '<select name="_wpsc_compare_'.$field_data->field_key.'" id="'.$field_data->field_key.'" style="width:400px">';
								echo '<option value="">'.__( "Select value", 'wp-ecommerce-compare-products' ).'</option>';
							if (is_array($field_option) && count($field_option) > 0) {
								foreach ($field_option as $option_value) {
									$option_value = trim(stripslashes($option_value));
									if ($option_value == $field_value) {
		...
		?>
        		<tr><td><i style="text-decoration:blink"><?php _e('Do not have any field on this category, please add fields for it at', 'wp-ecommerce-compare-products' ); ?> <a href="admin.php?page=wpsc-compare-features" target="_blank"><?php _e('this page', 'wp-ecommerce-compare-products' ); ?></a></i></td></tr>
        <?php	
		}
		?>
        	</tbody>
	...
	public static function save_compare_meta_boxes($post_id) {
		...
		if ($post_type == 'wpsc-product' && $post_status != false) {
			...
			if ( $category_data != NULL ) update_post_meta($post_id, '_wpsc_compare_category_name', stripslashes( $category_data->category_name ) );

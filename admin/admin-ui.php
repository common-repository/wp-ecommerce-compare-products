...
class WPEC_Compare_Admin_UI
{
	 ...
	 */
	public $admin_pages = array( 'wpsc-compare-features' );
	...
	public function upgrade_top_message( $echo = false, $setting_id = '' ) {
		$upgrade_top_message = sprintf( '<div class="pro_feature_top_message">'
			. __( 'Settings inside this yellow border are %s Features.', 'wp-ecommerce-compare-products' )
			. '<br />'
			. __( 'Upgrade to the <a href="%s" target="_blank">%s</a> to activate these settings.', 'wp-ecommerce-compare-products' )
			. '</div>'
			, apply_filters( $this->plugin_name . '_' . $setting_id . '_pro_version_name', apply_filters( $this->plugin_name . '_pro_version_name', __( 'Pro Version', 'wp-ecommerce-compare-products' ) ) )

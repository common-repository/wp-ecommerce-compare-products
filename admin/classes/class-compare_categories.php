...
class WPEC_Compare_Categories_Class
{
	...
	public static function init_categories_actions() {
		...
		if(isset($_REQUEST['bt_save_cat'])){
			...
			if(isset($_REQUEST['category_id']) && $_REQUEST['category_id'] > 0){
				...
				if ($category_name != '' && $count_category_name == 0) {
					...
					$cat_msg = '<div class="updated below-h2" id="result_msg"><p>'.__('Compare Category Successfully edited', 'wp-ecommerce-compare-products' ).'.</p></div>';
				}else {
					$cat_msg = '<div class="error below-h2" id="result_msg"><p>'.__('Nothing edited! You already have a Compare Category with that name. Use unique names to edit each Compare Category.', 'wp-ecommerce-compare-products' ).'</p></div>';
				}
			}else{
				...
				if ($category_name != '' && $count_category_name == 0) {
					...
					if ($category_id > 0) {
						$cat_msg = '<div class="updated below-h2" id="result_msg"><p>'.__('Compare Category Successfully created', 'wp-ecommerce-compare-products' ).'.</p></div>';
					}else {
						$cat_msg = '<div class="error below-h2" id="result_msg"><p>'.__('Compare Category Error created', 'wp-ecommerce-compare-products' ).'.</p></div>';
					}
				}else {
					$cat_msg = '<div class="error below-h2" id="result_msg"><p>'.__('Nothing created! You already have a Compare Category with that name. Use unique names to create each Compare Category.', 'wp-ecommerce-compare-products' ).'</p></div>';
				}
			}
		}
		...
		if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'cat-delete'){
			...
			$cat_msg = '<div class="updated below-h2" id="result_msg"><p>'.__('Compare Category deleted', 'wp-ecommerce-compare-products' ).'.</p></div>';
		}
		return $cat_msg;
	}
	public static function wpec_compare_categories_manager(){
		global $wpdb;	
		?>
        <h2><?php if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'cat-edit'){ _e('Edit Compare Product Categories', 'wp-ecommerce-compare-products' ); }else{ _e('Add Compare Product Categories', 'wp-ecommerce-compare-products' ); } ?></h2>
        <p><?php if(isset($_REQUEST['act']) && $_REQUEST['act'] != 'cat-edit'){ _e('Create Categories based on groups of products that share the same compare feature list.', 'wp-ecommerce-compare-products' ); } ?></p>
		...
		?>
        	<table class="form-table">
                <tbody>
                	<tr valign="top">
                    	<th class="titledesc" scope="rpw"><label for="category_name"><?php if(isset($_REQUEST['act']) && $_REQUEST['act'] == 'cat-edit'){ _e('Edit Category Name', 'wp-ecommerce-compare-products' ); } else { _e('Category Name', 'wp-ecommerce-compare-products' ); } ?></label></th>
            ...
            <p class="submit">
	        	<input type="submit" name="bt_save_cat" id="bt_save_cat" class="button button-primary" value="<?php if (isset($_REQUEST['act']) && $_REQUEST['act'] == 'cat-edit') { _e('Save', 'wp-ecommerce-compare-products' ); }else { _e('Create', 'wp-ecommerce-compare-products' ); } ?>"  /> <input type="button" class="button" onclick="window.location='admin.php?page=wpsc-compare-features'" value="<?php _e('Cancel', 'wp-ecommerce-compare-products' ); ?>" />
	    	</p>
        </form>
	<?php
	}
	...
	public static function wpeccp_update_cat_orders(){
		...
		_e('You just save the order for compare categories.', 'wp-ecommerce-compare-products' );
		die();
	}
}
?>
...
function wpec_compare_install(){
	update_option('a3rev_wpeccp_version', '3.0.0');
	$product_compare_id = WPEC_Compare_Functions::create_page( esc_sql( 'product-comparison' ), '', __('Product Comparison', 'wp-ecommerce-compare-products' ), '[product_comparison_page]' );
	...
	delete_transient("a3rev_wpec_compare_update_info");
...
function wpeccp_init() {
	...
	wpec_compare_plugin_textdomain();
}
...
add_action('plugin_action_links_' . ECCP_NAME, array( 'WPEC_Compare_Hook_Filter', 'settings_plugin_links' ) );
...
add_action('init', 'wpec_cp_pro_upgrade_plugin');
function wpec_cp_pro_upgrade_plugin () {

	// Upgrade to 3.0.0
	if(version_compare(get_option('a3rev_wpeccp_version'), '3.0.0') === -1){
		update_option('a3rev_wpeccp_version', '3.0.0');
